...
const RecommendedBook: React.FC<bookUrlProps> = ({ url }) => {
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 mt-8 xl:grid-cols-5 place-items-center gap-y-12 gap-x-12">
        {url?.recommended_books?.map((value: any, index: number) => {
          return (
            <div key={index}>
                  ...
                  <div className="relative w-60 h-72 mb-2">
                    {value?.cover && (
                      <Image
                        fill
                        sizes="fit"
                        src={`http://127.0.0.1:8000/${value?.cover}`}
                        alt="cover image"
                      />
                    )}
                  </div>
